...
if isinstance(current_view, str) and current_view.lower() in LEGACY_PROFILING_ALIASES:
...
def _get_page_from_query_params() -> Optional[str]:
    ...
    if candidate:
        ...
        if candidate_lower in LEGACY_PROFILING_ALIASES:
            return "profile"
    return None
...
if isinstance(page_state_value, str) and page_state_value.lower() in LEGACY_PROFILING_ALIASES:
    page_state_value = "profile"
    st.session_state["page"] = "profile"
if (
