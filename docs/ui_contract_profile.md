...
1. Page header labelled "🧪 Profiling" immediately followed by two captions: (a) the descriptive sentence "Run deterministic metadata-driven profiling backed by ZEUS_ANALYTICS_SIMU.DISCOVERY." and (b) the metadata note "Profiling metadata source: <namespace>."  No widgets may appear between these captions and the divider.
2. Stateless table picker with three selectors (database, schema, table) sourced from `stateless_table_picker`.  It must appear directly under the captions and persist the selected fully qualified name in `st.session_state["profile_target_fqn"]`.
3. A horizontal divider separating the picker from the controls.
4. Control row rendered as two equal columns:
...
5. When no table is selected, display the informational message "Use the database, schema, and table selectors above to choose a target table." and stop rendering additional sections.
6. Once a table is selected, show `st.success("Profiling target: <FQN>")` followed by a spinner that loads metadata using `services.profiling_v2` helpers.  After the spinner completes:
...
7. Results section uses `st.tabs` with four tabs in this exact order: "Column features", "Semantic tags", "Suggested checks", "Run history".  Tab labels cannot change.
...
8. When `DEBUG_PROFILING` is true, wrap the metadata payload in a single expander titled "Debug · Profiling payload".  The expander must be collapsed by default.
