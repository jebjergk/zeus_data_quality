    ...
    for chk in checks:
        ...
        if rule.upper().startswith(AGG_PREFIX):
            ...
            results["checks"].append({
                "check_id": chk.check_id, "type": chk.check_type, "aggregate": True,
                "ok": ok, "failures": (0 if ok else 1), "sample": []
            })
        else:
            df = session.sql(f"SELECT COUNT(*) FROM {chk.table_fqn} WHERE NOT ({rule})")
            failures = int(df.collect()[0][0])
            sample = []
            if chk.sample_rows and failures:
                s_df = session.sql(f"SELECT * FROM {chk.table_fqn} WHERE NOT ({rule}) LIMIT {int(chk.sample_rows)}")
                sample = [r.asDict() if hasattr(r,"asDict") else dict(r) for r in s_df.collect()]
            results["checks"].append({
                "check_id": chk.check_id, "type": chk.check_type, "aggregate": False,
                "failures": failures, "sample": sample
            })
    return results
